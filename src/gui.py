...
def create_course_check(tag, name, credits_input):
    ...
    elif functions.check_if_course_tag_free(tag) is True and credits_input.isdigit() is True:
    ...
    elif functions.check_if_course_tag_free(tag) is True and credits_input.isdigit() is False:
        Label(popup, text="Credits must be an integer", font=(FONT, 8, "bold")).place(x=10, y=150)
    elif functions.check_if_course_tag_free(tag) is False and credits_input.isdigit() is True:
...
def close_course_tag_check(tag):
    ...
    tag = tag.lower()
    if functions.check_if_course_tag_free(tag) is False:
